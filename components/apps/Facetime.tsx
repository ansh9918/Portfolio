...
export default function FaceTime({ isDarkMode = true }: FaceTimeProps) {
  ...
  useEffect(() => {
    let isMounted = true;
    // microtask defer prevents synchronous state update warning
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4 relative">
        {isCameraAvailable ? (
          <video
            ...
            autoPlay
            playsInline
            className="w-full max-w-2xl h-auto rounded-xl bg-black"
          />
        ) : (
        ...
        {isCameraAvailable && (
          <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2">
            <Button
              className={`w-16 h-16 rounded-full ${buttonBg} text-black`}
