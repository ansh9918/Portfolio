    ...
    const textElements = useMemo(
        () =>
            texts.map((text, index) => (
                <span
                    className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold px-2 sm:px-3 md:px-4 transition text-[#CCFFFF] duration-300 ease-in-out transform hover:scale-125 hover:shadow-cyan-600"
                    key={index}>
                    {text}
                </span>
            )),
    ...
    return (
        <div className="flex overflow-hidden whitespace-nowrap">
            ...
            <motion.div
                ...
                {textElements}
                {textElements}{" "}
                {/* Repeat the text elements to ensure continuous scrolling */}
            </motion.div>
        </div>
    );
};
